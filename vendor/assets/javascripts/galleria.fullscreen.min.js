  ...
  Galleria.addTheme({
    ...
    init: function(t) {
      ...
      this.bind("rescale", function() {
        r = this.getStageHeight() - e.height() - 2;
        n.css("top", l ? r - s.outerHeight() + 2 : r);
        var i = this.getActiveImage();
        if (i) {
          c(i)
        }
      });
      ...
      this.bind("image", function(i) {
        i.imageTarget && c(i.imageTarget)
      });
      this.bind("thumbnail", function(a) {
        i(a.thumbTarget).parent(":not(.active)").children().css("opacity", .6);
        i(a.thumbTarget).on("click:fast", function() {
          if (l && t._closeOnClick) {
            e.trigger("click:fast")
          }
        })
      });
      ...
      if (t._hideDock) {
        e.on("click:fast", this.proxy(function() {
          e.toggleClass("open", !l);
          if (!l) {
            n.animate({
              top: r - s.outerHeight() + 2
            }, 400, t.easing)
          } else {
            n.animate({
              top: r
